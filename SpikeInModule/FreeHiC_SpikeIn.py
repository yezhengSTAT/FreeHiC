...
def get_args():
    ...
    parser = argparse.ArgumentParser(description = '------------Usage Start------------',
                                     epilog = '------------Usage End------------')
    parser.add_argument('-s', '--spikein', help = 'Spike-in type. Two options: "data" for data-driven, "user" for user-define. The default is data-driven.', default = "data")
    ...
    parser.add_argument('-sp', '--percentage', help = 'The percentage of spike-ins (0-100) randomly sampled from the true differential set. 100 means the entire bin-pairs in the true set will be added as spike-ins. Default is 100.', default = 100)
...
if __name__ == "__main__":
    ...
    keyC1 = ["cond1_" + str(i) for i in range(1, len(cond1) + 1)]
    keyC2 = ["cond2_" + str(i) for i in range(1, len(cond2) + 1)]
    ...
    if spikeInType == "data":
        ...
        seqDep = rawFile[keyC1 + keyC2].sum(axis = 0)
        normFile = rawFile[keyC1 + keyC2]/seqDep * seqDep.max()
    ...
    elif spikeInType == "user":

        ## normalize by the sequencing depth
        seqDep = rawFile[keyC1].sum(axis = 0)
    ...
    for i in range(0, len(interFilePaths)):
        ...
        fragFinal = interFreq[~interFreq.index.isin(inInd.index)].append(fragFCleanZeroPosFrag[["freq", "fragA", "fragB"]]).sort_values(["fragA", "fragB"])
